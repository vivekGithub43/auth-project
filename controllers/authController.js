...
const User = require("../models/usersModles");
...
if(existingUser){
 return res.status(401).json({success:false,message:'user already exsit'})   
}
...
res.status(201).json({success:true,message:'your account has been createdd successfully',result})
  }catch(error){
...
if(!existingUser.verificationCode || !existingUser.verificationCodeValidation){
     return res.status(400).json({success:false,message:'something is wrong woth the code'})   
}
...
return res.status(401).json({success:false,message:'you are not verified user!'})
    ...
    if(!existingUser){
return res.status(401).json({success:false,message:'User doest exists'})
    }
